...
import { MatchState, Player, SetState } from './types';
...
import { Sun, Moon, Calendar, Trophy, Zap, DownloadCloud, RotateCcw, AlertCircle, HelpCircle } from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className={`min-h-screen font-sans transition-all duration-300 ${
      ...
      {/* GLOBAL HEADER */}
      <header className={`border-b py-4 px-4 transition-all duration-300 shadow-card ${
        sunMode 
        ...
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className={`w-11 h-11 rounded-xl flex items-center justify-center font-black shadow-card border transition-all duration-300 ${
              sunMode ? 'bg-charcoal-900 text-white border-sand-200' : 'bg-primary text-slate-950 border-secondary-border'
            }`}>
              BH
            </div>
            <div>
              <h1 className={`text-xl font-black tracking-tight uppercase flex items-center gap-2 ${
                sunMode ? 'text-slate-900' : 'text-white'
              }`}>
                BeachHandball Stats
              </h1>
              <p className={`text-[10px] font-semibold uppercase tracking-wider ${
                sunMode ? 'text-slate-600' : 'text-slate-300'
              }`}>
          ...
          <div className={`flex gap-5 items-center py-2 px-5 rounded-xl border transition-all duration-300 shadow-sm ${
            sunMode 
            ...
            <div className="text-center">
              <span className={`block text-[9px] uppercase font-black tracking-wider ${
                sunMode ? 'text-slate-500' : 'text-zinc-400'
              }`}>SET 1</span>
              ...
              <span className={`block text-[8px] font-black uppercase ${
                sunMode ? 'text-slate-700' : 'text-zinc-300'
              }`}>
            ...
            <div className={`h-8 border-r ${
              sunMode ? 'border-sand-200' : 'border-zinc-800'
            }`} />
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`p-2.5 rounded-lg border flex items-center justify-center gap-1.5 text-xs font-black uppercase transition-all duration-300 active:scale-[0.98] shadow-sm ${
                sunMode 
            ...
            <button
              ...
              className={`p-2.5 rounded-lg border font-black flex items-center gap-1.5 text-xs transition-all duration-300 active:scale-[0.98] shadow-sm ${
                sunMode 
                  ? 'border-sand-200 bg-white text-slate-900 hover:bg-sand-50/50' 
                  : 'border-zinc-700 bg-charcoal-900 text-white hover:bg-charcoal-800'
              }`}
      ...
      <nav className={`py-1 px-4 border-b transition-colors duration-300 shadow-card ${
        sunMode ? 'bg-white border-sand-155' : 'bg-charcoal-900 border-charcoal-800'
      }`}>
        <div className="max-w-7xl mx-auto flex justify-around sm:justify-start gap-1 sm:gap-4 overflow-x-auto">
          <button
            ...
            className={`py-3 px-4 rounded-lg font-bold text-sm uppercase flex items-center gap-2 border-b-2 transition-all duration-300 active:scale-[0.98] ${
              activeTab === 'pista'
      ...
      {showResetConfirm && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-xs p-4">
          <div className={`p-6 rounded-2xl max-w-md w-full shadow-2xl border-2 transform scale-100 transition-all ${
            sunMode 
            ...
            <div className="flex gap-3 justify-end">
              <button
                ...
                className={`py-2.5 px-4 rounded-xl font-bold text-xs uppercase cursor-pointer hover:opacity-85 transition active:scale-95 duration-100 ${
                  sunMode ? 'bg-[#e5dfd3] text-zinc-800' : 'bg-zinc-800 text-zinc-100'
                }`}
              >
                Cancelar
              </button>
      ...
      <footer className={`fixed bottom-0 left-0 right-0 py-2.5 px-4 text-center select-none transition-colors ${
        sunMode ? 'bg-background text-charcoal-500' : 'bg-background text-charcoal-500'
      }`}>
